...
public class EditReportActivity extends AppCompatActivity {
    ...
    private void updateReport() {
        ...
        // Buat data untuk approval request
        Map<String, Object> editData = new HashMap<>();
        editData.put("originalReportId", currentReport.getId());
        editData.put("title", title);
        editData.put("description", description);
        editData.put("amount", amount);
        editData.put("date", selectedDate.getTime());

        // Proses upload gambar jika ada
        if (selectedImageUri != null) {
            uploadImageAndCreateEditRequest(editData);
        } else {
            createEditRequest(editData);
        }
    }

    private void uploadImageAndCreateEditRequest(Map<String, Object> editData) {
        StorageReference storageRef = storage.getReference()
                .child("edit_request_images/" + UUID.randomUUID().toString());

        storageRef.putFile(selectedImageUri)
                .addOnSuccessListener(taskSnapshot -> {
                    storageRef.getDownloadUrl().addOnSuccessListener(uri -> {
                        editData.put("imageUrl", uri.toString());
                        createEditRequest(editData);
                    });
                })
    ...
    private void createEditRequest(Map<String, Object> editData) {
        FirebaseUser currentUser = mAuth.getCurrentUser();

        ApprovalRequest editRequest = new ApprovalRequest(
        ...
        db.collection("approvalRequests")
                .document(editRequest.getId())
                .set(editRequest)
                .addOnSuccessListener(aVoid -> {
                    Toast.makeText(this, "Permintaan edit sedang diproses", Toast.LENGTH_SHORT).show();
                    finish();
                })
                .addOnFailureListener(e -> {
                    Toast.makeText(this, "Gagal membuat permintaan edit", Toast.LENGTH_SHORT).show();
                });
    }
